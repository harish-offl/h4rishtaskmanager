...
export default function Clients() {
  ...
  const save = () => {
    ...
    if (editId) {
      setClients(cs => cs.map(c => c.id === editId ? { ...form, id: editId } : c))
      toast.success('Client updated')
    } else {
      setClients(cs => [{ ...form, id: uid(), createdAt: new Date().toISOString() }, ...cs])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[['Total', clients.length],['Active', clients.filter(c=>c.status==='Active').length],['Total Revenue', formatINR(clients.reduce((a,c)=>a+Number(c.totalRevenue||0),0))],['Pending', formatINR(clients.reduce((a,c)=>a+Number(c.pending||0),0))]].map(([l,v]) => (
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead>
              <tr><th>Client</th><th>Company</th><th>Phone</th><th>Services</th><th>Manager</th><th>Project Value</th><th>Revenue</th><th>Pending</th><th>Status</th><th>Actions</th></tr>
            </thead>
            <tbody>
